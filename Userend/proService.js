document.addEventListener('DOMContentLoaded',function(){
    fetch('http://localhost:5500/GetAllPro')
...
document.querySelector('table tbody').addEventListener('click',function(event){
   ...
   if(event.target.className === "delete-btn"){
       deleteproId(event.target.dataset.id);
   }
});
...
function deleteproId(id){
   fetch('http://localhost:5500/DeletePro/' + id,{
       method:'DELETE'
   })
   .then(response => response.json())
   .then(data =>{
...
const searchBtn = document.querySelector('#search-pro-btn');
searchBtn.onclick = function(){
    console.log(searchBtn)
    const searchValue = document.querySelector('#search-pro-input').value
    fetch('http://localhost:5500/SearchPro/' + searchValue)
    .then(response => response.json())
    .then(data =>  loadHTMLTable(data['data']));
