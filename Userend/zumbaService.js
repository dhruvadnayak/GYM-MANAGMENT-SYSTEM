document.addEventListener('DOMContentLoaded',function(){
    fetch('http://localhost:5500/GetAllZumba')
...
document.querySelector('table tbody').addEventListener('click',function(event){
   ...
   if(event.target.className === "delete-btn"){
       deletezumbaId(event.target.dataset.id);
   }
});

//delete row from user

function deletezumbaId(id){
   fetch('http://localhost:5500/DeleteZumba/' + id,{
       method:'DELETE'
   })
   .then(response => response.json())
   .then(data =>{
...
const searchBtn = document.querySelector('#search-zumba-btn');
searchBtn.onclick = function(){
    console.log(searchBtn)
    const searchValue = document.querySelector('#search-zumba-input').value
    fetch('http://localhost:5500/SearchZumba/' + searchValue)
    .then(response => response.json())
    .then(data =>  loadHTMLTable(data['data']));
